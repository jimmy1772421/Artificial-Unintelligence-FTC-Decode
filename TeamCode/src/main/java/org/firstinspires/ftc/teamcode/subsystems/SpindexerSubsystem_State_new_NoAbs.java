...
public class SpindexerSubsystem_State_new_NoAbs {
    ...
    private static final double ABS_MECH_OFFSET_DEG = (104.1); // calibrate
    ...
    // Global safety so auto never deadlocks
    ...
    // ==== ABS INIT-LOOP ONLY GATING ====
    // ABS is allowed ONLY before lockAbsAfterInit() / first update()
    private boolean absLocked = false;
    private double absRawDegCached = Double.NaN;
    private double absInternalDegCached = Double.NaN;
    ...
    public static boolean ENABLE_ABS_CORRECTIONS = false;

    public SpindexerSubsystem_State_new_NoAbs(HardwareMap hardwareMap) {
        ...
        // IMPORTANT:
        // No ABS reads here. We assume "whatever ticks are now" is our temporary zero
        // until init_loop calibration aligns ticks to ABS.
        zeroTicks = motor.getCurrentPosition();
        intakeIndex = 0;

        // Start by holding slot 0 at intake
    ...
    /** Call from OpMode.init_loop(). Safe to call repeatedly. Reads ABS ONLY if not locked. */
    public void initLoopCalibrateFromAbs() {
        if (absLocked) return;

        // Read ABS raw 0..360 and cache it
        double absRaw = readAbsAngleDeg_0to360();
        // internalAngle = 0 when absRaw == ABS_MECH_OFFSET_DEG
        double internalAngle = normalizeAngle(absRaw - ABS_MECH_OFFSET_DEG);
        absInternalDegCached = internalAngle;
        ...
        // Hold whatever intake slot we believe is at intake right now
        targetAngleDeg = slotCenterAngleAtIntake(intakeIndex);
    }

    /** Call from OpMode.start() (recommended). update() also locks automatically. */
    public void lockAbsAfterInit() {
        absLocked = true;
    }

    /** ABS raw 0..360. Reads only before lock; after lock returns cached. */
    private double readAbsAngleDeg_0to360() {
        if (absLocked) return absRawDegCached;
    ...
    private Ball detectBallColor() {
        ...
        RevColorSensorV3 chosen = (present1 && present2) ? ((d1 <= d2) ? intakeColor : intakeColor2)
    ...
    public boolean update(Telemetry telemetry,
        ...
        // HARD GUARANTEE: once update() runs, ABS is locked forever.
        // This ensures no ABS reads during match even if someone calls debug methods.
        absLocked = true;

        long now = System.currentTimeMillis();

        // Sync PIDF from Panels every loop
        setPIDF(
        ...
        if (effectiveTagOverride == 21 || effectiveTagOverride == 22 || effectiveTagOverride == 23 || effectiveTagOverride == 0) {
            gameTag = effectiveTagOverride;
        }

        // Start eject sequence on rising edge
        if (yEdge && !ejecting) {
            if (hasAnyBall()) {
                ejecting = true;
                ...
                // Fallback: pretend we have balls, based on tag/pattern or default PURPLE
                Ball[] pattern = getPatternForTag(gameTag);
                if (pattern != null) {
        ...
        if (ejecting) {
            switch (ejectState) {
                case ROTATING_TO_LOAD: {
                    ...
                    // timeout -> tick-only recovery (NO ABS)
    ...
    public boolean isAtMid() {
        // kept from your old code (uses ticks only)
    ...
    /** Raw abs angle 0..360 (ONLY reads before lock; after lock returns cached). */
    public double dbgAbsRawDeg() {
        return readAbsAngleDeg_0to360();
    }

    /** Internal abs angle (slot0@intake = 0), 0..360 (cached after lock). */
    public double dbgAbsInternalDeg() {
        if (absLocked) return absInternalDegCached;
        double raw = readAbsAngleDeg_0to360();
        absInternalDegCached = normalizeAngle(raw - ABS_MECH_OFFSET_DEG);
        return absInternalDegCached;
    }
    ...
    public void debugAbsAngle(Telemetry telemetry) {
        // This will NOT read ABS after lock; it will show cached values.
        double raw = dbgAbsRawDeg();
        double corrected = dbgAbsInternalDeg();
