...
public class ShooterSubsystemNoPID {
    ...
    // ===== STATE =====
    private boolean isOn = false;     // True if the subsystem is currently commanded to run
    ...
    public ShooterSubsystemNoPID(HardwareMap hardwareMap) {
    ...
    public void update(boolean shooterOnCommand,
        ...
        int newFieldPos = (fieldPosInput == 1) ? 1 : 0; // clamp to [0,1]
        if (newFieldPos != fieldPos) {
            fieldPos = newFieldPos;

            // Move hood when mode changes
            if (fieldPos == 1) { // far field
